...
namespace Utils
{
    ...
    std::string statusCode(DWORD ex_code)
    {
        switch(ex_code)
        {
            // TO BUILD THIS FUNCTION with newer ntstatus.h, paste the result of this command :
            // for i in `grep -niR "define *STATUS" /cygdrive/c/CODEBLOCKSMINGW/include/ddk/ntstatus.h| grep -v STATUS_SEVERITY |grep -v STATUS_WAIT_[0-9]|grep -vw STATUS_ABANDONED|cut -d' ' -f2`; do echo case $i: return \"$i\"\;; done
